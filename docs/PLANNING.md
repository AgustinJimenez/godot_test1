...
| 2026-07-19 | Shambler strikes require facing within 12 degrees, lock body yaw through wind-up/contact/recovery, and use a translation-only placeholder lunge. | Continuously steering toward a strafing player during the strike compounded with the placeholder's whole-model pitch, producing an unnatural rotating swivel. Attacks now commit to a direction like authored melee animation, while chase turning resumes after recovery. |
...
| 2026-07-19 | Melee damage now lands at one normalized contact point emitted by the one-shot body animation instead of immediately on input. Shambler attacks use wind-up, contact-time range validation, and recovery; until a compatible zombie strike clip is imported, a short visual-child lunge telegraphs that contact without moving collision. `Health.apply_damage()` returns and signals actual applied damage, including capped overkill. | Separating input, animation timing, hit discovery, and health arithmetic makes interrupted or escaped attacks miss correctly, guarantees one hit per swing, and keeps each responsibility replaceable. Moving only the enemy visual during the placeholder telegraph preserves navigation/collision authority and can be removed when a real authored clip is available. |
| 2026-07-18 | Added three shamblers with Ch10/zombie-girl visuals to the nature sandbox on a runtime-baked navmesh and added item-defined melee damage/range through a composed `MeleeWeapon` capsule query; Q uses `Sword_Attack` for equipped KayKit weapons and remains a weak punch when unarmed. `Shambler.character_scene` selects any compatible Mixamo visual without changing its AI scene, and `character_bone_prefix` rewrites its small action-pack animation set for numbered-prefix rigs such as Ch10's `mixamorig5_`. | This reuses the established `Health` discovery contract for both player and enemy damage, keeps combat behavior out of `Item` resources, and makes one query work for first/third-person because aim remains camera-driven while the held model remains skeleton-driven. A capsule covering the complete reach is more forgiving and avoids the close-range dead zone produced by a sphere placed only at maximum reach. Separating the visual scene from the archetype avoids duplicating navigation/perception/state logic for each zombie skin. |
