...
| Date | Decision | Why |
|---|---|---|
| 2026-07-28 | Organize Character Creator outfit fitting as a subsystem under `ui/outfit_fit/`. `editor.gd` remains the stateful Godot `Node3D` façade and public API, while the synchronous fitting pipeline, clipping visualization/cache, preview materials, topology components, layer ordering, geometry queries, sampling, and profile encoding live in focused helpers. Solver helpers receive shared mesh state plus rebuild/projection callbacks rather than owning UI nodes or signals. | Outfit fitting grew to 1,300 lines in one editor script and mixed interaction state with body projection, collision cleanup, garment layering, debug mesh construction, and materials. Extracting those responsibilities reduced the façade to 938 lines and Character Creator to 967 lines, restored the project-wide 1,000-line lint ceiling, and preserved the existing `OutfitFitEditor` API and Godot resource UIDs. |
| 2026-07-28 | Auto Fit preserves thickness and silhouette for topology components classified as solid shells. Pairing uses only local mesh geometry—a three-median-edge search radius and opposing accumulated face normals—and requires at least 80% vertex coverage so partial inner faces on ordinary garments are not treated as solids. Small local groups include coincident seam copies, share their average automatic displacement after every body/collision/layer stage, and undergo eight group-aware topology smoothing passes. Components below the coverage threshold retain the existing per-vertex cloth behavior. | Projecting every vertex independently onto a body-clearance envelope collapsed the Peasant leather belt's inner and outer walls: its median separation fell from 7.59 mm to 4.19 mm and overlapping faces produced dark triangular marks. A shared local displacement retains authored thickness (7.62 mm after the same selected fit) without an asset name or globally rigid component transform. Merging seam copies into the groups prevents the seam pass from undoing thickness equality, and topology smoothing reduced the isolated belt's 95th-percentile neighboring offset jump from 2.13 mm to 1.20 mm (maximum 6.62 mm to 2.73 mm). The belt and buckle qualify as solid shells; classifying partial wall regions in the shirt, pants, sleeves, or boots caused severe whole-outfit breakthroughs, so those remain on the ordinary cloth path. |
| 2026-07-28 | Component inspection in Edit Fit uses a render-only `Isolate selected component` toggle. It hides the base body and rebuilds outfit previews from only the selected topology component's triangle indices; disabling it rebuilds the complete preview. The toggle is unavailable for `All surfaces` and `All components`, and control-point visibility remains independent. | Logical pieces such as the Peasant leather belt share one imported mesh surface with the shirt, buckle, and trim, so hiding other `MeshInstance3D` nodes or surfaces cannot expose the belt alone. Index filtering isolates the actual fitting unit without mutating imported assets, deformation offsets, or saved profiles, letting an author distinguish source-mesh artifacts from Auto Adjust artifacts. |
| 2026-07-28 | Layer-aware outfit fitting derives inner/outer order from each component's imported signed body distance and creates graph edges only where two components occupy the same body-space cells. It resolves and rebuilds that directed graph one level at a time after body clearance, letting inner corrections feed outer layers. Surfaces are split into position-welded disconnected components, allowing embedded pieces such as the Peasant belt and buckle to layer independently from the shirt despite sharing one material/surface. The Edit Fit panel exposes those pieces in a dependent Component selector whose default `All components` retains whole-surface behavior. Component selection scopes controls, debug rendering, manual influence, and Auto Adjust. Actual cross-surface triangle intersections move the authored outer layer outward; same-surface components enforce body-relative depth order using the four closest samples within 6 cm and a 2 mm separation. After every body, collision, and layer stage, position-welded duplicate vertex indices receive one averaged offset so UV/hard-normal seams cannot split. Selected-only fitting preserves scope: a selected inner layer can move inward only while retaining body clearance. | A single global average can compare unrelated regions such as boots and sleeves, while forcing every garment onto one body-clearance shell erases the shirt-over-pants and pants-over-boots offsets already authored into the pack. An overlap-only graph represents the actual local stack and avoids inventing relationships between unrelated pieces; rebuilding between graph levels makes Body → Shirt → Leather belt → Metal buckle an executable order rather than metadata only. Moving only intersecting belt vertices left shirt patches, while pushing the whole component or using a neighboring-cell maximum visibly inflated it; local interpolation closes sampling gaps while better preserving the accessory silhouette. Exposing the same topology identity to the UI makes embedded accessories independently tunable without changing imported assets or crowding the main surface list. Independent final collision pushes had separated 51 coincident pants seam groups by up to 1.29 cm, creating a non-collision hole; synchronizing their offsets preserves authored seam continuity. |
| 2026-07-27 | Gross leg fitting uses dominant skin-bone regions to keep left/right garment vertices on the corresponding body limb. Leg-dominant surfaces use an outside-in body ray, cap each correction pass at 3 cm, and smooth displacement over triangle adjacency plus position-welded seams; torso and arm surfaces retain nearest-surface fitting. Corrections are seeded only by cloth/body triangle intersections or garment vertices measured inside the matching body region, and the final intersection cleanup is limited to 16 small passes. | Unconstrained nearest points could select the neighboring leg and independently stretch adjacent pants/boot vertices; applying outside-in rays and smoothing to every region then introduced new torso artifacts and took about 30 seconds. Restricting the technique to its actual failure area restored the shirt and kept pants/boots smooth in front and oblique captures. Clipping-only seeds preserve noncontact geometry in principle, but this grossly mismatched body/outfit genuinely marks 2,538 vertices versus 2,696 with minimum-clearance fitting, so the visible and performance gain is modest (about 11.4 seconds versus 10.5). A thin outer-thigh body line remains, making this an experimental improvement rather than complete collision removal. |
| 2026-07-27 | Vendor GPU Cloth Sim at upstream commit `bd917af` only as an isolated Male Peasant manual harness, not as an enabled editor plugin or Character Creator runtime dependency. The trial keeps garments on their authored skeleton, drives a hidden body collider in the same rest frame, and enables mutual shirt/pants collision with procedural vertex-color weights. | The compute solver holds the static fit and a gentle spine twist, and its peer proxies initialize correctly. Directly rebinding clothes to the body distorted them because corresponding rest landmarks differ by up to 3.4 cm; broader thigh motion still destabilizes the procedurally weighted pants. The upstream solver also reports invalid RenderingDevice resources during shutdown on Godot 4.6.2/Metal. Authored cloth masks/topology and a lifecycle fix must pass an animated test before gameplay integration. |
| 2026-07-27 | Manual outfit-fit controls use a shared, direction-aware 4 cm surface grid and expose one nonnegative body-normal distance instead of XYZ. Auto Adjust treats the user-configurable 0.1–5 cm clearance (5 mm default) as a minimum: it only pushes deficient garment vertices outward and never pulls already-clear vertices inward, then repeatedly resolves intersecting interior cloth triangles. Open-rim detection welds coincident positions before counting edges and exempts only triangles wholly on a real rim; the per-vertex layer persists with the body/outfit profile. | A scalar distance expresses surface contact directly, while a whole-outfit grid avoids the seam clusters caused by equal per-material quotas. An exact shared target shell erased authored shirt-over-pants and pants-over-boots height differences; outward-only correction retains those source layer offsets while still enforcing body clearance. The denser grid gives roughly four times the surface controls of the original 8 cm spacing without changing saved-handle identity. Raw vertex-index boundaries also misclassified ordinary UV/hard-normal splits as holes and excluded broad leg bands from collision correction, so position welding preserves real collars/cuffs/tears while checking triangles that merely touch their rims. |
| 2026-07-26 | Prototype body/outfit-specific manual fit profiles in Character Creator: sparse selectable garment-surface handles apply smooth rest-space vertex offsets to a duplicated mesh, persist by stable body/outfit IDs, preserve imported skin weights, and recolor clothing vertices beneath the body surface green during diagnostic preview. | Automatic nearest-surface clearance can expose or inflate the wrong regions and cannot express an art-directed twist. A reusable sparse deformation profile makes the tuned values inspectable and repeatable, while live penetration coloring gives each edit immediate spatial feedback without modifying source assets; animation-pose validation is still required because rest-shape edits cannot correct bad clothing weights. |
