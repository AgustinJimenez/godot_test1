...
> **Status (2026-07-16):** M0–M4 done. The unarmed walk clip (retargeted from Universal Animation Library) has been reverted - frozen arms/spine read as too stiff in actual play - so the player is back to the single native MotusMan aim-pose walk for now (see Decisions Log); a proper unarmed walk is unscheduled pending a better retargeting approach. The shambler (patrol/investigate/chase/attack/search, `NavigationAgent3D` + vision cone + hearing) is live in `test_room.tscn` on a runtime-baked navmesh. Next up: **M5 — Survival pressure** (flashlight battery, scarcity tuning, safe-room save/load). Main scene is `levels/playground.tscn`; controls: WASD / Shift sprint / C crouch / F flashlight / E interact / Tab inventory / LMB fire / RMB aim / R reload / V debug camera / P debug menu.
