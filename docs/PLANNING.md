...
| 2026-07-27 | Manual outfit-fit controls use a shared, direction-aware 8 cm surface grid and expose one nonnegative body-normal distance instead of XYZ. Auto Adjust separately contact-fits every garment vertex to its nearest outside body surface with a user-configurable 0.1–5 cm clearance (5 mm default), then repeatedly pushes intersecting interior cloth triangles outward while exempting intentional open attachment seams; the per-vertex layer persists with the body/outfit profile. | A scalar distance expresses surface contact directly, while a whole-outfit grid avoids the seam clusters caused by equal per-material quotas. Solving only visible controls left interpolated triangles inside the body, so automatic fitting operates at vertex resolution and reserves the grid for manual refinement. Signed projection handles both loose and deeply embedded vertices; configurable clearance accounts for coarse curved surfaces and shallow raster-visible breakthroughs that a 1 mm numerical tolerance can miss. |
