...
| Date | Decision | Why |
|---|---|---|
| 2026-07-28 | Layer-aware outfit fitting derives pairwise inner/outer order from each garment's imported signed body distance, sampled only in body-space cells occupied by both surfaces. After body clearance, actual garment triangle intersections move the authored outer layer outward. Selected-only fitting preserves scope: a selected inner layer can move inward only while retaining body clearance. | A single global average can compare unrelated regions such as boots and sleeves, while forcing every garment onto one body-clearance shell erases the shirt-over-pants and pants-over-boots offsets already authored into the pack. Shared-region sampling preserves the relevant local order, and pairwise intersection tests avoid the reverted prototype's unconditional pushes. |
