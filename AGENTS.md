...
When retargeting animation between differently-authored skeletons, **never transfer raw local rotation deltas or world-space swing rotations directly between rigs**. A bone pose is only meaningful relative to that rig's parent rest frame; skipping that conversion caused the UAL arm spread, wrist freeze, twisted knees, shallow crouch/sitting, and floating collapse poses. Use Godot 4.6 `RetargetModifier3D`'s model-space local-pose formula: `target_parent_global_rest^-1 * source_parent_global_rest * source_pose * source_rest^-1 * source_parent_global_rest^-1 * target_parent_global_rest * target_rest`. The synchronous bake implementation is `_humanoid_retarget_local_pose()` in `player_body.gd`; pelvis position deltas also use the parent-rest conversion plus the target/source hip-height ratio. Keep `levels/_pose_compare.tscn` until manual confirmation: its menu plays the same clip on retargeted MotusMan and the untouched UAL source for free-camera comparison from multiple angles. The full failed-approach history is in `docs/task_history/ual_animation_retargeting.md`.

Retargeted animations preserve the source clip's `loop_mode`; do not force every imported clip to loop. Only aliases listed in `UAL_LOOPING_GAMEPLAY_CLIPS` explicitly override the source with `LOOP_LINEAR`; jump start/landing and other one-shots must remain `LOOP_NONE`. The pose-comparison scene switches with zero blend time so short one-shot poses start in sync; the in-game debug menu keeps its normal 0.2-second crossfade.
