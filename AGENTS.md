...
Retargeted animations preserve the source clip's `loop_mode`; do not force every imported clip to loop. Only the semantically continuous clips in `UAL_GAMEPLAY_CLIPS` explicitly override the source with `LOOP_LINEAR`. The pose-comparison scene switches with zero blend time so short one-shot poses start in sync; the in-game debug menu keeps its normal 0.2-second crossfade.
