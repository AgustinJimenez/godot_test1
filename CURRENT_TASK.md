...
`tools/outfit_pipeline/body_region_mask.gd` (`BodyRegionMask`) is a utility that does this (not yet integrated into the preview — see Status above for why). It rebuilds the body mesh by dropping fully-hidden triangles. Its exemption mechanisms are described below; once the garment-opening detection is complete, it can replace the discard-shader baseline.
