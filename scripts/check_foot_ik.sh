...
godot --headless --fixed-fps 60 --path "$project_dir" \
	res://tests/manual/foot_ik/foot_ik_spacing_plan_check.tscn \
	--quit-after 5 >"$log_file" 2>&1 || true
if rg -q "SCRIPT ERROR" "$log_file" \
		|| ! rg -q "FOOT_IK_SPACING_PLAN_CHECK PASS cases=39" "$log_file"; then
	cat "$log_file"
	exit 1
fi
...
if rg -q "SCRIPT ERROR" "$log_file" \
		|| ! rg -q "FOOT_IK_SLOPE_TARGET_LIFECYCLE_CHECK PASS" "$log_file"; then
	cat "$log_file"
	exit 1
fi
